/**
 * Segmented (flag-on) MovementSystem tests.
 *
 * Mocks `@/config/feature-flags` at the file scope so every test in this
 * file observes `SEGMENTED_PATHS_ENABLED = true`. Paired with
 * `MovementSystem.test.ts` (flag off) which remains the default-flag
 * coverage until the legacy branch is removed in Phase 7.
 */
import { describe, it, expect, afterEach, vi } from 'vitest'
import { GamePhase } from '@/data/constants'

vi.mock('@/config/feature-flags', () => ({
  SEGMENTED_PATHS_ENABLED: true,
}))
...
describe('MovementSystem (segmented pipeline)', () => {
  ...
  it('falls back to the legacy branch gracefully when levelContext is absent', () => {
    const advance = vi.fn((s) => s)
    registerStrategy('linear', { advance })
    registerStrategy('horizontal', { advance })
    ...
    expect(advance).not.toHaveBeenCalled()
    // Legacy arc-length branch moved the enemy.
    expect(enemy._pathX).toBeLessThan(6)
  })
})
