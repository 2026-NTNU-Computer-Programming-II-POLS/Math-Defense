/**
 * Flag-on TowerPlacementSystem tests (P3-T10).
 *
 * Exercises the `SEGMENTED_PATHS_ENABLED`-gated delegation to
 * `PlacementPolicy`. Rule-level correctness of the policy itself is
 * covered in `domain/level/placement-policy.test.ts`; this file asserts
 * only that the system (a) routes the decision to the policy, (b) emits
 * `PLACEMENT_REJECTED` with the policy's reason on denial, and (c) still
 * performs the post-placement bookkeeping (gold, TOWER_PLACED) on accept.
 */
import { describe, it, expect, vi } from 'vitest'

vi.mock('@/config/feature-flags', () => ({ SEGMENTED_PATHS_ENABLED: true }))
...
describe('TowerPlacementSystem (segmented paths enabled)', () => {
